...
def test_status_line1_basic():
    ...
    assert "hottest: нудьга 0.90" in line
...
def test_needs_panel_one_row_per_need():
    ...
    assert len(rows) == 2  # самотність + нудьга
    assert any(r.lstrip().startswith("самотність") for r in rows)
    assert any("нудьга" in r for r in rows)


def test_needs_panel_marks_hottest_and_shows_threshold():
    rows = needs_panel_lines(_snap(hottest=["novelty", 0.9]))
    novelty = next(r for r in rows if "нудьга" in r)
...
def test_needs_panel_below_threshold_no_flag():
    ...
    connection = next(r for r in rows if "самотність" in r)
...
def test_needs_panel_shows_action():
    rows = needs_panel_lines(_snap())
    assert "→ deep" in next(r for r in rows if "нудьга" in r)
    assert "→ chat" in next(r for r in rows if "самотність" in r)
