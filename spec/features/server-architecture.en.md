...
- **Permission scope** (the field that later gates tools) is attached to each agent here but **not yet
  enforced** — enforcement is v1.7 (Tools). Designing the field now keeps v1.7 additive.
...
- **RAG** recall over transcripts → **v1.6**.
- A typed-argument **tool registry** + per-agent permission **enforcement** → **v1.7**.
...
9. **Second agent — Pashu (§12).** Author a minimal `state/pashu/` (its own `canon.md` + needs / mood
   ...
   permission-scope field (enforced in v1.7). The remote TUI (step 7) can attach to either agent.
...
- **A narrower permission scope.** **Agnika = home / elevated**; **Pashu = a companion with a narrower
  scope**. The scope field is set per agent in v1.2; **enforcement** arrives with tools in **v1.7** —
...
between two agents. The capability itself arrives later — it gets its own phase, 1.8, built on the tool
system from 1.7 — since "send a message to another agent" is an action, and actions are governed by
...
locked-down companion. This arrives in phase 1.8.
...
This is also part of phase 1.8.
...
from phase 1.12) that can manage simultaneous writes correctly, rather than a shared JSON file.
...
natural order is: in 1.2 the agents are isolated; in 1.8 direct messages and observation appear, as a
permission-controlled tool, and one TUI can hold several agents at once; shared memory comes only after
the move to PostgreSQL in 1.12; and that shared substrate then powers the group-chat rooms in 1.13, where
